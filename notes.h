...
/**
 ...
 =(赋值) &(取地址) ,(序列)运算符应用于类对象时已经有了预先定义的意义，可以将其定义为私用
 来禁止使用他们，也可以重新定义新的意义。
 
 在使用指针或者引用时，才能使用多态。
 
 用一个类作为基类，相当于声明一个该类的（匿名）对象，所以基类必须先定义，而不只是声明。
 同理，子类也必须调用基类的构造函数，显示或隐式地，默认构造函数可以被隐式地调用。
 ...
 如果没有定义赋值运算符，编译器会生成一个。这也意味着赋值运算符是不继承的。构造函数也不。
